...
// Duplication-gate fixture: the same offsets as Declared.cc, in a file that
// also holds a function. The rule keys on where each *site* lies, not on
// whether its file contains code somewhere — so this family is rejected exactly
// like the one in `declarations/`.

#include <cstdint>

namespace fixture::computed {
...
std::uint64_t headerEnd() {
	return kNodeCountOffset + sizeof(std::uint64_t);
}
