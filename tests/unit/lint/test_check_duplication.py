...
The fixtures are `.cc`, not `.cpp`. The naming contract (AGENTS.md §1) is what
every gate walking the tree keys on, so a deliberately duplicated fixture must
not be a source file by that contract — it is gate input, like the JSON under
...
class ReportingTest(unittest.TestCase):
    def test_a_shared_block_is_reported_with_both_of_its_sites(self):
        ...
        self.assertGreaterEqual(block.tokens, 40)
    ...
    def test_the_same_pair_passes_under_a_threshold_above_the_block(self):
        report = check_duplication.duplicate_blocks(_tree("pair"), min_tokens=400)
