...
class Removals(AdrGateTest):
    ...
    def test_a_superseded_record_may_not_be_rewritten_afterwards(self):
        ...
        self.assertEqual(outcome.returncode, 1, outcome.stdout + outcome.stderr)
        self.assertIn("Decision", outcome.stderr)
        # And the message must say which status it actually had. "while the ADR
        # is Accepted" was hard-coded, so the one case where the distinction
        # matters reported the wrong thing.
        self.assertIn("while the ADR is Superseded", outcome.stderr)
