...
# STORY-0302: FAT32 geometry and directory entries
...
and no traversal — [story-0303](story-0303-fat32-cluster-chains.md) walks the
...
- [story-0102](story-0102-ntfs-boot-sector.md) — the NTFS boot-sector parser is
...
3. **Names** (`src/fs/fat/ShortName.cpp`, `src/fs/fat/LongName.cpp`) — the 8.3
   ...
   extracted for the assembler in story-0303.
...
the mount table would move on (story-0301: a mounter that recognizes its own
signature owns the answer). So the string is what story-0303's mounter checks
...
back with `lossless == false`, which is what story-0303 grades `kUncertain` on.
...
directory walk that story-0303 builds. This story stops at "these 13 code units,
...
- [x] A `BS_FilSysType` other than `FAT32   ` is `kInvalidArgument` at offset
      `0x52`; `fat::filSysTypeIsFat32` exposes the same check to story-0303's
