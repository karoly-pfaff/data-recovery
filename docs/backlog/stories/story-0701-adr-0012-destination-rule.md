...
length and in the right place. Restoring is itself an edit to an `Accepted` ADR, and it is
the one edit the rule has to permit: reverting an unauthorised change is what makes the
record immutable rather than merely append-only. The story records that reasoning so
[story-0705](story-0705-adr-immutability-check.md)'s check can be written to allow it.
...
- [ ] `docs/architecture/adr/adr-0012-*.md` exists, `Accepted`, and states: the two tiers
...
- [ ] ADR-0012 declares that it supersedes ADR-0011's Validated half, naming it.
- [ ] ADR-0011 carries a `Superseded by ADR-0012` marker on that half; its original text
      is otherwise unchanged.
- [ ] ADR-0005's Consequences read exactly as `5079837` accepted them — verified by
      `git show 5079837:docs/architecture/adr/adr-0005-read-only-by-default.md`, not by eye.
- [ ] No `Accepted` ADR contains a statement about the destination rule that contradicts
      `src/recovery/DestinationRule.cpp`.
- [ ] The ADR index lists ADR-0012 and shows ADR-0011's new status.
- [ ] Every code citation added by this story names a symbol, not a line number.
...
- **The superseded marker is real.** `story-0705`'s check, once it exists, is the
  permanent form of this. Until then: grep ADR-0011 for the marker and for the absence of
  any remaining "story-0609 exists to" future-tense claim.
...
- [ ] Acceptance criteria met, tests green (ASan + UBSan) — unchanged by this story, so
...
- [ ] Docs/ADRs updated — this story *is* that item.
