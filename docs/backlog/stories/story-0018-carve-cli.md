...
# STORY-0018: `revenant-carve` CLI — format allowlist, destination
...
[story-0017](story-0017-undelete-cli.md) that is not specific to it moves into
...
- [story-0025](story-0025-plausibility-filter.md) — the allowlist is applied at
...
3. **The shared frontend layer**, extracted from story-0017 rather than copied:
   ...
   - `cli/RecoveryRun` — story-0017's run, now driven by a `RunRequest` that
...
So story-0017's `UndeleteRun` becomes `RecoveryRun`, its options parser splits
...
- [x] `revenant-undelete` behaves exactly as it did in story-0017; its tests are
...
story-0065 fixture image — carved JPEGs under `carved/jpg/` and no reconstructed
