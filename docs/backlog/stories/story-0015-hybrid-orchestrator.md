...
# STORY-0015: Hybrid orchestrator — FS pass → byte accounting → carve pass
...
the metadata can name ([story-0013](story-0013-ntfs-entry-enumeration.md)), then
carve the space those names did not account for
([story-0014](story-0014-carver-registry-and-scan.md)) — which is the claim
...
- The candidate index and arbitration ([story-0019](../epic-m1-vertical-slice.md)).
  ...
  guesswork.
- Extraction, naming, and dedup (story-0016) — nothing is written.
...
- [x] `scan(device, visitor)` is `scanRegion` over the whole device, so
      story-0014's behaviour is unchanged (its tests are untouched).
...
Integration (`tests/integration/HybridRecoveryTest.cpp`): the story-0065 fixture
