...
# STORY-0113: Output safety — `sanitizeOutputPath` + `boundedCount`
...
4. **`OutputPathSegment.{hpp,cpp}` is an internal, non-public pair** under
   ...
   `WindowMatch.hpp` established for `SignatureScanner` (story-0107) and
   `JpegMarkerWalk.hpp`/`JpegEntropyScan.hpp` established for `JpegCarver` (story-0103):
...
6. **The release-preset `tidy` run drove a further decomposition pass beyond the first
   ...
   instruction story-0107 documented. The first draft compiled and passed all 108 tests
   but the full `clang-tidy` run (`readability-function-size`, `StatementThreshold: 10`,
   which — as story-0107 first found — counts every statement recursively, including
