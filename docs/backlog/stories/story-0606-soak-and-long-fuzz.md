...
- **`make_seed_corpus.py` regenerates the tracked seeds.** It regenerates every
  ...
  *finds*, and a generator has no business claiming to write them. That is now
  ...
  The other is a real drift, and this story's own audit disputed it, so it was
  measured again with both sides captured to separate files first — committed
  bytes from `git show`, generated bytes from a run of the script — rather than
  by comparing a working-tree file that a revert had already put back.
  `Ext4EnumerateFuzz/volume.bin`: 65,536 bytes each, SHA-256 `99928d3a…` against
  `3c6a3cf7…`, and exactly two bytes differ — offsets
