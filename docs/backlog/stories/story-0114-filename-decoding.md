...
# STORY-0114: NTFS filename decoding (UTF-16) + safe/disambiguated output names
...
alongside story-0113's `sanitizeOutputPath` in `revenant::recovery` (the destination/sink
...
- [ADR-0009: output safety — path confinement & bounded allocation](../../architecture/adr/adr-0009-output-safety.md)
  (`disambiguate` is step 4 of the same pipeline `sanitizeOutputPath`, story-0113, opened)
...
2. **`decodeUtf16Name` escapes a literal NUL code unit unconditionally**, even though
   ...
   limitation: `sanitizeOutputPath` (story-0113) already rejects any raw NUL byte in a
...
4. **Same "MSVC has no local `clang++`" gap as story-0113's `OutputPathFuzz.cpp`.**
...
- [x] No new type introduced needs DIP — both primitives are free functions over value
      ...
      and story-0113's "leaf primitive" shape; `disambiguate`'s `taken` parameter is
...
- [x] YAGNI: no configurable escape format, no alternate surrogate policy, no
      ...
      `kMaxSegmentBytes`/`kMaxSegments` are (story-0113): tests need to assert *at* the
...
- [x] `decodeUtf16Name` — the byte-parsing entry point this story adds that consumes
      ...
      target of its own, matching the same scoping story-0113 used for
